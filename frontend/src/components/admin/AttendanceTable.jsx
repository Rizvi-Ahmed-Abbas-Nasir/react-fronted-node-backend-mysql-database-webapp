            ...
            <tr>
              ...
              {attendanceData.length > 0 &&
                Object.keys(attendanceData[0].events).map((event, index) => (
                  <th key={index} className="border px-4 py-2">{event}</th>
                ))}
            </tr>
          </thead>
          <tbody>
            {attendanceData.map((student) => (
              <tr key={student.student_id}>
                ...
                <td className="border px-4 py-2">{`${student.first_name} ${student.middle_name || ''} ${student.last_name}`}</td>
                {Object.values(student.events).map((status, index) => (
                  <td key={index} className="border px-4 py-2">
                    {status === 1 ? 'Present' : status === 0 ? 'Absent' : 'Not Registered'}
                  </td>
                ))}
              </tr>
            ))}
